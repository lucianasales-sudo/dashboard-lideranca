import streamlit as st
import pandas as pd
import plotly.express as px

st.set_page_config(page_title="Dashboard Liderança", layout="wide")

@st.cache_data
def load_data():
    try:
        # Lendo o arquivo e ignorando linhas com erro (como a 93)
        df = pd.read_csv('dados2_lideranca.csv', 
                         sep=';', 
                         encoding='latin-1', 
                         on_bad_lines='skip', 
                         engine='python')
        # Limpando espaços nos nomes das colunas
        df.columns = [str(c).strip().upper() for c in df.columns]
        return df
    except Exception as e:
        st.error(f"Erro ao carregar o CSV: {e}")
        return None

# Execução do App
df = load_data()

if df is not None:
    st.title("📊 Painel de Resultados")
    
    # Identificando colunas automaticamente
    try:
        col_lider = [c for c in df.columns if 'LIDER' in c][0]
        col_nota = [c for c in df.columns if 'NOTA' in c][0]
        col_nome = [c for c in df.columns if 'NOME RH' in c or 'NOME INVOLVES' in c][0]

        # Tratando a nota (vírgula para ponto)
        df[col_nota] = df[col_nota].astype(str).str.replace(',', '.')
        df[col_nota] = pd.to_numeric(df[col_nota], errors='coerce').fillna(0)

        # Filtro na lateral
        lista_lideres = sorted(df[col_lider].unique())
        lider_sel = st.sidebar.selectbox("Selecione o Supervisor", lista_lideres)
        
        df_filtrado = df[df[col_lider] == lider_sel]

        # Mostrando os números
        c1, c2 = st.columns(2)
        c1.metric("Equipe", len(df_filtrado))
        c2.metric("Média Nota", f"{df_filtrado[col_nota].mean():.1f}")

        # Criando o gráfico
        fig = px.bar(df_filtrado, x=col_nome, y=col_nota, title=f"Desempenho: {lider_sel}")
        st.plotly_chart(fig, use_container_width=True)
        
        st.write("### Detalhes da Equipe")
        st.dataframe(df_filtrado)
        
    except Exception as e:
        st.error(f"Erro ao processar as colunas: {e}")
        st.write("Colunas encontradas:", list(df.columns))
